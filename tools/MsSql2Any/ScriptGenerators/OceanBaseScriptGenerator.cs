...
public class OceanBaseScriptGenerator : IScriptGenerator
{
    public string DbType => "OceanBase";
    ...
    public string GenerateCreateTableScript(string tableName, List<ColumnInfo> columns)
    {
        ...
        foreach (var col in columns)
        {
            ...
            if (col.IsIdentity)
                columnDef += " AUTO_INCREMENT"; // OceanBase uses AUTO_INCREMENT for identity columns
    ...
    public string GenerateInsertScript(string tableName, List<ColumnInfo> columns, IEnumerable<object[]> dataRows)
    {
        ...
        foreach (var row in dataRows)
        {
            ...
            // For OceanBase, if there are identity columns, we might want to specify column names
    ...
    {
        // OceanBase data type mapping, compatible with MySQL mode
        return column.DataType.ToLower() switch
        {
            ...
            "nchar" => $"CHAR({Math.Max(1, column.MaxLength)})", // In OceanBase, NCHAR is the same as CHAR
            "varchar" => column.MaxLength > 0
                ? $"VARCHAR({column.MaxLength})"
                : "TEXT",
            "nvarchar" => column.MaxLength > 0
                ? $"VARCHAR({column.MaxLength})" // In OceanBase, NVARCHAR is the same as VARCHAR
                : "TEXT",
            "text" => "TEXT",
            "ntext" => "LONGTEXT", // In OceanBase, use LONGTEXT
            "binary" => $"BINARY({column.MaxLength})",
            "varbinary" => column.MaxLength > 0
                ? $"VARBINARY({column.MaxLength})"
                : "LONGBLOB",
            "image" => "LONGBLOB",
            "uniqueidentifier" => "CHAR(36)", // In OceanBase, use CHAR(36) to store UUID
            "xml" => "LONGTEXT",
            "timestamp" => "TIMESTAMP",
            "rowversion" => "BINARY(8)",

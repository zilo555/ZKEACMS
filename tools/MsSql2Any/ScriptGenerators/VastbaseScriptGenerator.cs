...
public class VastbaseScriptGenerator : IScriptGenerator
{
    public string DbType => "Vastbase";
    ...
    public string GenerateInsertScript(string tableName, List<ColumnInfo> columns, IEnumerable<object[]> dataRows)
    {
        ...
        foreach (var row in dataRows)
        {
            ...
            // For Vastbase, if there are identity columns, we might want to specify column names
    ...
    {
        // Vastbase data type mapping, based on PostgreSQL
        return column.DataType.ToLower() switch
        {
            "int" => "INTEGER",
            "bigint" => "BIGINT",
            "smallint" => "SMALLINT",
            "tinyint" => "SMALLINT", // Vastbase does not have TINYINT, use SMALLINT
            "bit" => "BOOLEAN", // Vastbase uses BOOLEAN instead of BIT
            ...
            "nchar" => $"CHAR({Math.Max(1, column.MaxLength)})", // In Vastbase, NCHAR is the same as CHAR
            "varchar" => column.MaxLength > 0
                ? $"NVARCHAR({column.MaxLength})"
                : "TEXT",
            "nvarchar" => column.MaxLength > 0
                ? $"VARCHAR({column.MaxLength})" // In Vastbase, NVARCHAR is the same as VARCHAR
                : "TEXT",
            "text" => "TEXT",
            "ntext" => "TEXT", // In Vastbase, there is no NTEXT, use TEXT
            "binary" => $"BYTEA", // In Vastbase, use BYTEA to store binary data
            "varbinary" => $"BYTEA",
            "image" => "BYTEA",
            "uniqueidentifier" => "UUID", // Vastbase has UUID type
            "xml" => "TEXT", // Vastbase uses TEXT to store XML
            "timestamp" => "BYTEA", // In Vastbase, TIMESTAMP is datetime type, not row version

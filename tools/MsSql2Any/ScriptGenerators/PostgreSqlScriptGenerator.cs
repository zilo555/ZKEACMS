...
public class PostgreSqlScriptGenerator : IScriptGenerator
{
    ...
    private string MapDataType(ColumnInfo column)
    {
        ...
        return column.DataType.ToLower() switch
        {
            ...
            "nchar" => $"CHAR({Math.Max(1, column.MaxLength)})", // In PostgreSQL, NCHAR is the same as CHAR
            "varchar" => column.MaxLength > 0
                ? $"VARCHAR({column.MaxLength})"
                : "TEXT",
            "nvarchar" => column.MaxLength > 0
                ? $"VARCHAR({column.MaxLength})" // In PostgreSQL, NVARCHAR is the same as VARCHAR
                : "TEXT",
            "text" => "TEXT",
            "ntext" => "TEXT", // In PostgreSQL, there is no NTEXT, use TEXT
            "binary" => $"BYTEA", // In PostgreSQL, use BYTEA to store binary data
            "varbinary" => $"BYTEA",
            "image" => "BYTEA",
            "uniqueidentifier" => "UUID", // PostgreSQL has UUID type
            "xml" => "XML",
            "timestamp" => "BYTEA", // In PostgreSQL, TIMESTAMP is datetime type, not row version
